...
- `OSL/N` = one-shot layer → NUM. `GUI/F` = tap GUI, hold → FN layer. `Sh` = `OSM(MOD_LSFT)` (tap one-shot shift, hold for normal held shift). `T/N` = LTNAV (tap T, hold → NAV). `←/Ct`, `→/Alt` = mod-tap arrows. `SPC/S` = tap space, hold → SYM. `⌘Lock` = `OS_LOCK` (macOS lock screen, `Cmd+Ctrl+Q`). `Scrn` = `Cmd+Shift+Ctrl+4` (macOS region screenshot to clipboard).
...
**NUM** (one-shot from `OSL/N`) — numpad-style digits on left hand (`6 4 0 2` / `8` and `3 1 5 7` / `9`), `+` plus `.`/`SPC`/`BSPC`/`-` on thumbs.
...
- **Language toggle (`CRYLTG` / `SN_ESC_CRYL`)** — `CRYLTG` (top-right BASE key) enables Russian: fires `Ctrl+Space` (macOS input-source-switch shortcut), turns CRYL layer on. Only fires if CRYL isn't already active (so a stray repeat can't desync firmware from OS). `SN_ESC_CRYL` (combo: `S`+`N`) is Esc normally, but disables Russian (fires `Ctrl+Space` again + `layer_off`) when CRYL is active.
