...
- `OSL/N` = one-shot layer → NUM. `GUI/F` = tap GUI, hold → FN layer. `Sh` = `OSM(MOD_LSFT)` (tap one-shot shift, hold for normal held shift). `T/N` = LTNAV (tap T, hold → NAV). `←/Ct`, `→/Alt` = mod-tap arrows. `SPC/S` = tap space, hold → SYM. `⌘Lock` = `OS_LOCK` (macOS lock screen, `Cmd+Ctrl+Q`).
